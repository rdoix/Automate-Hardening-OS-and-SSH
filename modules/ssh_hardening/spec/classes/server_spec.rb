...
describe 'ssh_hardening::server' do
  ...
  # default configuration
  expect_option('ssh::server', 'PasswordAuthentication', 'no')
  # user configuration
  context 'with pam_auth => true' do
    let(:params) { { :pam_auth => true } }
    expect_option('ssh::server', 'PasswordAuthentication', 'yes')
  end
  context 'with pam_auth => false' do
    let(:params) { { :pam_auth => false } }
    expect_option('ssh::server', 'PasswordAuthentication', 'no')
  end
